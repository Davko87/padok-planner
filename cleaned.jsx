    ...
    <>
      ...
      >
        <div className="glass-panel flex flex-col h-full overflow-hidden shadow-glass-lg border-white/25">
          {/* Header */}
          <div className="p-4 border-b border-white/15 flex items-center justify-between bg-transparent">
            {!isCollapsed && (
                      ...
                      <div className="pl-2 flex flex-col gap-1 overflow-hidden">
                        ...
                        <div className="flex items-center gap-1.5 text-lg text-white/80">
                          {team.elements && team.elements.length > 0 ? (
                            team.elements.map((el, idx) => {
                              let icon = '📦';
                              if (el.type === 'truck') icon = '🚛';
                              if (el.type === 'awning') icon = '⛺';
                              if (el.type === 'van') icon = '🚐';
                              if (el.type === 'tent') icon = '🎪';
                              if (el.type === 'car') icon = '🚗';
                              if (el.type === 'towTruck') icon = '🛻';
                              
                              return (
                                <span key={idx} title={el.type} className="bg-white/10 px-1.5 py-0.5 rounded-md border border-white/10 flex items-center justify-center shadow-sm">
                                  {icon}
                                </span>
                              );
                            })
                          ) : (
                            <span className="bg-white/10 px-1.5 py-0.5 rounded-md border border-white/10 flex items-center justify-center shadow-sm" title="truck">
                              🚛
                            </span>
                          )}
